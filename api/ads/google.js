...
async function gaqlSearch(query) {
  ...
  const d = await res.json();
  if (d.error) throw new Error(`Google Ads GAQL: ${JSON.stringify(d.error)}`);
  return d.results || [];
}
...
async function getInsights(level = 'campaign', dateRange = 'last_30_days') {
  ...
  return rows.map(r => {
    ...
    return {
      ...
      cpc:         +(parseInt(m.average_cpc || 0) / 1_000_000).toFixed(2),
      cpm:         +(parseInt(m.average_cpm || 0) / 1_000_000).toFixed(2),
...
async function getCampaigns() {
  ...
  return rows.map(r => ({
    ...
    objective:   r.campaign?.advertising_channel_type || '',
    dailyBudget: r.campaign_budget?.amount_micros
                   ? parseInt(r.campaign_budget.amount_micros) / 1_000_000
                   : null,
...
async function mutateCampaign(body) {
  const c     = creds();
  const token = await getAccessToken();
  ...
  const d = await res.json();
  if (d.error) throw new Error(`Google Ads mutate: ${JSON.stringify(d.error)}`);
  return d;
}

async function pauseCampaign(id) {
  await mutateCampaign({ operations: [{ update: { resourceName: `customers/${creds().customerId}/campaigns/${id}`, status: 'PAUSED' }, updateMask: 'status' }] });
  return { success: true };
}

async function resumeCampaign(id) {
  await mutateCampaign({ operations: [{ update: { resourceName: `customers/${creds().customerId}/campaigns/${id}`, status: 'ENABLED' }, updateMask: 'status' }] });
  return { success: true };
}
...
async function setBudget(campaignId, dailyBudgetUsd) {
  ...
  const d = await res.json();
  if (d.error) throw new Error(`Google SetBudget: ${JSON.stringify(d.error)}`);
  return { success: true };
}
...
async function createCampaign(campaign) {
    ...
    body: JSON.stringify({
      operations: [{
        create: {
          ...
          amountMicros:  Math.round((campaign.budget?.daily || 50) * 1_000_000),
  ...
  if (budgetData.error) throw new Error(`Google CreateBudget: ${JSON.stringify(budgetData.error)}`);
    ...
    body: JSON.stringify({
      operations: [{
        create: {
          name:                    campaign.name,
          status:                  'PAUSED',
          advertisingChannelType:  OBJ_MAP[campaign.objective] || 'SEARCH',
          campaignBudget:          budgetRN,
          startDate:               (campaign.schedule?.start || new Date().toISOString().slice(0, 10)).replace(/-/g, ''),
          endDate:                 campaign.schedule?.end?.replace(/-/g, '') || undefined,
          manualCpc:               {},
        },
      }],
    }),
  ...
  if (campData.error) throw new Error(`Google CreateCampaign: ${JSON.stringify(campData.error)}`);
